...
public class BinaryUtil
{
    ...
    [SqlFunction( Name = "Splx_ContainsOne" )]
    ...
    [SqlFunction( Name = "Splx_GetTableOr", DataAccess = DataAccessKind.Read )]
    public static SqlBinary GetTableOr(SqlString tableName, SqlString maskFieldName)
    {
        byte[] mask = new byte[] { };
        ...
        using( SqlConnection connection = new SqlConnection( "context connection=true" ) )
        {
            ...
            SqlCommand top1 = new SqlCommand( string.Format( "SELECT TOP 1 {1} FROM {0}", tableName.Value, maskFieldName.Value ), connection );
            byte[] maskField = top1.ExecuteScalar() as byte[];
            mask = new byte[maskField.Length];
            int bitArrayLength = maskField.Length * 8;
